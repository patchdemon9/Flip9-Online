...
const ROOM_EXPIRY_MS = 1000 * 60 * 60 * 12;
...
function addAction(room, text, type = "info") {
  room.recentActions = [
    { text, type, at: Date.now() },
    ...(room.recentActions || [])
  ].slice(0, 8);
}

function addReaction(room, playerName, emoji) {
  room.reactions = [
    { playerName, emoji, at: Date.now() },
    ...(room.reactions || [])
  ].slice(0, 10);
}
...
function beginRound(room) {
  ...
  room.recentActions = [];
  room.reactions = [];
  addAction(room, `Round ${room.roundNumber} began.`, "round");
}
...
function finishRound(room) {
  if (room.transitionLocked || room.phase !== "playing") {
    return;
  }

  room.transitionLocked = true;

  for (const player of room.players) {
    const earned = roundScore(player);
    player.stats.bestRound = Math.max(player.stats.bestRound, earned);
...
io.on("connection", socket => {
  socket.on("createRoom", ({ name }, reply) => {
    ...
    const room = {
      ...
      players: [{
        ...
        colour: "#f8c85f",
        stats: { flips: 0, banks: 0, busts: 0, bestRound: 0, perfects: 0 }
      }]
    };
  ...
  socket.on("joinRoom", ({ name, code }, reply) => {
    ...
    const palette = ["#f8c85f","#6dbafc","#5ee092","#ff8080","#b153c6","#ff9f43","#45d6c8","#ff6fae","#8f7cff","#a3d65c"];
    ...
    room.players.push({
      ...
      busted: false,
      completedFlip9: false,
      colour: palette[room.players.length % palette.length],
      stats: { flips: 0, banks: 0, busts: 0, bestRound: 0, perfects: 0 }
    });
    ...
    addAction(room, `${cleanName} joined the room.`, "join");
  ...
  socket.on("reconnectPlayer", ({ code, token }, reply) => {
    ...
    addAction(room, `${player.name} reconnected.`, "join");
  ...
  socket.on("flipCard", reply => {
    ...
    player.stats.flips += 1;

    if (player.roundCards.includes(card)) {
      player.busted = true;
      player.stats.busts += 1;
      room.message = `${player.name} flipped another ${card} and busted.`;
      addAction(room, `${player.name} busted on ${card}.`, "bust");
      addAction(room, `Achievement: ${player.name} earned "Painfully Predictable".`, "achievement");
      advanceTurn(room);
    } else {
      player.roundCards.push(card);
      const greedyComments = [
        `${player.name} risked it and found a ${card}.`,
        `${player.name} survives with a ${card}.`,
        `A ${card}! Probability blinks first.`,
        `${player.name} keeps the dream alive with ${card}.`
      ];
      room.message = greedyComments[Math.floor(Math.random() * greedyComments.length)];
      addAction(room, `${player.name} flipped ${card}.`, "flip");

      if (new Set(player.roundCards).size === 9) {
        player.completedFlip9 = true;
        player.banked = true;
        player.stats.perfects += 1;
        room.message = `${player.name} completed Flip 9 and earned a ${FLIP_9_BONUS}-point bonus.`;
        addAction(room, `${player.name} completed Flip 9!`, "perfect");
        addAction(room, `Achievement: ${player.name} earned "Nine Lives".`, "achievement");
      }

      // One action per turn: after a single flip, play always passes
  ...
  socket.on("bankScore", reply => {
    ...
    player.banked = true;
    player.stats.banks += 1;
    player.stats.bestRound = Math.max(player.stats.bestRound, roundScore(player));
    room.message = `${player.name} banked ${roundScore(player)} points.`;
    addAction(room, `${player.name} banked ${roundScore(player)} points.`, "bank");

    if (player.roundCards.length >= 6) {
      addAction(room, `Achievement: ${player.name} earned "Nerves of Steel".`, "achievement");
    }
    advanceTurn(room);
  ...
  socket.on("nextRound", reply => {
    ...
    // A rapid double tap can send the request twice. The first request
    // starts the round; later duplicates are harmlessly ignored.
    ...
    if (room.phase !== "roundOver") {
      reply?.({ ok: false, error: "The current round is not ready to advance." });
      return;
    }
  ...
  socket.on("sendReaction", ({ emoji }, reply) => {
    const room = getRoom(socket);
    const player = getSocketPlayer(socket, room);
    const allowed = ["👏","😂","😭","😈","🔥","😱","💀","👀"];

    if (!room || !player || !allowed.includes(emoji)) {
      reply?.({ ok: false });
      return;
    }

    addReaction(room, player.name, emoji);
    reply?.({ ok: true });
    emitRoom(room);
  });
  ...
  socket.on("disconnect", () => {
    ...
    // Keep the player and room alive so a refresh, weak mobile signal,
    // backgrounded browser tab or brief Render/WebSocket interruption
    // does not destroy the room code.
    player.connected = false;
    player.socketId = null;
    ...
    if (room.phase === "playing" && room.currentPlayerToken === player.token) {
      player.busted = true;
      room.message = `${player.name} disconnected and was skipped.`;
      addAction(room, `${player.name} disconnected and was skipped.`, "bust");
      advanceTurn(room);
    } else {
      room.message = `${player.name} temporarily disconnected.`;
      addAction(room, `${player.name} temporarily disconnected.`, "info");
    }
